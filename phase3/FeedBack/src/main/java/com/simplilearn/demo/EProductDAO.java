...
public class EProductDAO {
	...
	public List<EProductEntity> getProducts() {
		return template.query("select * from feedback", new RowMapper<EProductEntity>() {
	...
	public int addProduct(EProductEntity product) {
		return template.update("INSERT INTO feedback (ID, Name, feedback,dateAdded) VALUES(?,?,?,?)",
				new Object[] { product.getID(), product.getName(), product.getfeedback(), product.getDateAdded() });
	}

	public EProductEntity getByName(String name) {
		EProductEntity entity = template.queryForObject("select * from feedback where name=?",
